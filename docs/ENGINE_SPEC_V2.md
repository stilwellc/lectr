...
**Step 8 (deferred, in priority order):** (a) watches era/recency gate — finish `watches-backtest2.ts`/`watches-keytype-err.ts` before shipping anything (5–10× era drift is the best candidate for the 39% BM hold, but unmeasured); (b) Goldin sold history → client shards or server-side band precompute vs the 507K R2 corpus (worth ~5× the game-used parser's live yield); (c) crawl-stamp dimensions + design modelKey (unlocks the currently-blind 2.2× length gate: fires on 29.8% of pairs when it can see, sees 0.28%); (d) meteorite weight parsing (the biggest unexploited within-identity lever for science bands).
