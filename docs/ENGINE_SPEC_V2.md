...
Measured LOO on the exact-card tier (198,494 keys, 40,765 leave-one-out targets, strictly-earlier priors): plain median of ALL sales medAbsLogErr 0.354 (×1.425), 40.5% within ±30% → 1y-decay weighted median **0.325 (×1.385), 42.9% within ±30%**; the new 15/85 dispersion band covers the realized price on **46.0%** of targets where the old `low = high = value` band covered ~0% (2.6% of pools are still degenerate at n=2, where the band is [min,max]).
...
- `scripts/validate-engine.ts`: markets derived from `ARTISTS`; runs through `backtest-core.valueOne` (production replay path); gates G1–G4 (see file header); exits 1 on failure; `--sample`, `--market`, `--json`. G2's per-market monotonicity carries a sampling-error tolerance — a drop only counts when it exceeds max(5pt, 2 SE of the difference) — plus a ≥5pt bottom→top spread; a strict 1pt rule reds on n≈40 noise every night, which is how a gate dies. Local run (`--sample 6000`, 4,950 holdout lots, 101 s): coverage 67%, art +23pt / design +30pt / watches +39pt bottom→top, **exit 0**; the same run under the old strict rule exited 1 on an art n=61→n=105 noise dip (both exit paths verified).
...
- **TCG comp tier** — measured against the Aug 25 live book (465 bid-only Pokémon lots) using the Aug 14 sold pool: **88 seat a value (45 `tcg-exact`, 43 `tcg-grade-adj`)**, up from 0/586; 246 carry no parseable pokemonKey (`abstain: 'no-identity'`) and 131 have a key but a pool under 2 (`tcg:pool<2`). LOO on the exact tier (3,768 targets): medAbsLogErr 0.265 (×1.30), 50% within ±30%. The local corpus snapshot has zero upcoming Pokémon rows, so the in-build counter printed `0/0` — the harness above is the receipt. `build-market.ts` §3e: `pokemonKey` (year|set|#no|edition|GRADE) exact pool (n ≥ 2, 1y decay, dispersion band; n ≥ 4 'high' else 'medium', tier `tcg-exact`) and a cross-grade pool adjusted with the SPORTS grade ladder as a proxy (tier `tcg-grade-adj`, capped 'low', no vsBid) until a Pokémon ladder is fitted. Counts are logged as `[market] tcg value estimator`.
