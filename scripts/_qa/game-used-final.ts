...
function recoverPlayerSlug(title: string): string | null {
  ...
  for (; i < toks.length; i++) {
    const lw = toks[i].normalize('NFD').replace(/[̀-ͯ]/g, '').toLowerCase()
      ...
      .replace(/['’]s$/, ''); // possessive: "Jordan's" → jordan
...
console.log('spot:', recoverPlayerSlug('Shaquille O’Neal Phoenix Suns 2008-2009 Game Worn Sneakers'));
console.log('spot:', recoverPlayerSlug('Michael Jordan’s Game Worn 1985 Player Sample Air Jordan 1s | Sizes 13, 13.5'));
console.log('spot:', recoverPlayerSlug("ja'kobe walter ‘rookie debut’ toronto raptors 2024-2025 game worn icon edition jersey"));
console.log('spot:', recoverPlayerSlug("isaiah jackson 'playoff debut' indiana pacers 2024 nba eastern conference round 1 game worn jersey"));
...
type Gate = 'sportsForm' | 'objectType' | 'objectType2';
// objectType2 = objectType with the expanded ball rule ('other' rows whose
// titles name a baseball/basketball/football and no other gear noun → 'ball')
function keyOf(l: AuctionLot, gate: Gate): string {
  if (gate === 'sportsForm') return sportsForm(l) ?? classifyForm(l);
  const ot = l.objectType ?? 'other';
  if (gate === 'objectType2' && ot === 'other'
    && /\b(baseball|basketball|football|soccer ball)\b/i.test(l.title || '')
    && !/\b(jersey|helmet|cap|hat|glove|cleat)\b/i.test(l.title || '')) return 'ball';
  return ot;
}
function simBand(lot: AuctionLot, itemGate: Gate, excludeSelf: boolean) {
  const id = idOf(lot);
  if (!id) return { band: null as null | { med: number; n: number; spread: number }, why: 'noId' };
  const key = keyOf(lot, itemGate);
  const raw = guSold.filter(l => (!excludeSelf || l.id !== lot.id) && idOf(l) === id &&
    keyOf(l, itemGate) === key);
...
for (const gate of ['sportsForm', 'objectType', 'objectType2'] as const) {
