...
/** Read the full corpus (gz first, then raw, then the slim served files as a
 *  last-resort fallback for a fresh checkout). */
export function readCorpus(): Record<string, unknown>[] {
  const read = (base: string): Record<string, unknown>[] | null => {
    ...
    const served = path.join(SERVED_DIR, base);
    if (fs.existsSync(served)) return JSON.parse(fs.readFileSync(served, 'utf8'));
    return null;
  };
