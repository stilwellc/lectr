...
export default function GlobalError({ error, reset }: { error: Error; reset: () => void }) {
  ...
  return (
    <html lang="en">
      <body style={{ margin: 0, background: '#F4F5F0', color: '#1D1D1B', fontFamily: 'Inter, system-ui, sans-serif' }}>
        <main style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 4, padding: 24, textAlign: 'center' }}>
          ...
          <p style={{ fontSize: 14, color: '#6B6B65', margin: '10px 0 22px', maxWidth: 400 }}>
            The desk hit an unexpected error. Reloading usually clears it.
          </p>
          <div style={{ display: 'flex', gap: 10 }}>
            <button onClick={() => reset()} style={{ background: '#1D1D1B', color: '#F4F5F0', border: 'none', padding: '9px 18px', borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
              Try again
            </button>
            <a href="/" style={{ display: 'inline-flex', alignItems: 'center', padding: '9px 18px', borderRadius: 8, fontSize: 14, border: '1px solid rgba(29,29,27,0.18)', color: '#1D1D1B', textDecoration: 'none' }}>
              Back to the market
            </a>
          </div>
        </main>
